...
// Update the renderPoints function to show ONLY location name without point number
function renderPoints(location) {
    const container = document.getElementById('points-container');
    container.innerHTML = '';
    ...
    if (pointInfo) {
        // Find the location name from point value
        const locationDisplayName = Object.entries(POINT_MAPPINGS).find(([loc, point]) => 
            point === pointInfo.point)?.[0] || 'Location not found';

        const div = document.createElement('div');
        div.className = 'point-item';
        div.innerHTML = `
            <span class="point-value">${locationDisplayName}</span>  <!-- Only show the location name -->
            <button class="edit-btn" onclick="showEditPointModal('${location}')">Edit Point</button>
        `;
        container.appendChild(div);
    } else {
...
// Update the select dropdown population
function populatePointDropdown() {
    ...
    // Add only location names to dropdown
    Object.entries(POINT_MAPPINGS).forEach(([location, point]) => {
        const option = document.createElement('option');
        option.value = point;  // Keep point value in value attribute (hidden)
        option.textContent = location;  // Show only location name
        select.appendChild(option);
    });
}

// Update showEditPointModal to select the correct location name
function showEditPointModal(locationName) {
    ...
    // Find and select the correct location in dropdown
    const select = document.getElementById('edit-point-value');
    select.value = pointInfo.point;  // Set value but display shows only location name
...
function showAddPointModal(locationName) {
    ...
    populatePointDropdown(); // Add this line
